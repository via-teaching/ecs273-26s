...
export default function LineChart({ selectedStock }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8000/stock/${selectedStock}`)
      .then((res) => {
        if (!res.ok) {
          throw new Error(`Failed to fetch data for stock ${selectedStock}`);
        }        return res.json();
      })
      .then((apiResponse) => {
        const rawData = apiResponse.stock_series;
        ...
        if (data.length === 0) {
          console.warn(`No valid data points for stock ${selectedStock}`);
          return;
        }
