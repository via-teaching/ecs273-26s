      ...
      .then((data) => {
        ...
        const points = (data.points ?? [])
          .map((p) => ({
            ...
            x: +p.x,
            y: +p.y,
            sector: (p.sector ?? "Unknown").trim(),
          }))
          .filter((p) => p.ticker && !Number.isNaN(p.x) && !Number.isNaN(p.y));

        console.log("Loaded t-SNE points:", points.length);
