...
import { fetchStockPrices } from "../api.js";

function recordsToChartRows(records) {
  return (records ?? [])
    .map((r) => {
...
function getDateFormatter(zoomK) {
  if (zoomK < 2) return d3.timeFormat("%b %Y");
  if (zoomK < 6) return d3.timeFormat("%b %d");
  return d3.timeFormat("%a %b %d");
}

function getTickCount(zoomK, chartWidth) {
  const density = chartWidth / 72;
  if (zoomK < 2) return Math.min(6, Math.max(4, Math.floor(density / 2)));
  if (zoomK < 6) return Math.min(10, Math.max(5, Math.floor(density)));
  return Math.min(18, Math.max(8, Math.floor(density * Math.sqrt(zoomK))));
}

function getLabelRotation(zoomK) {
  if (zoomK < 2) return 0;
  if (zoomK < 6) return -35;
  return -40;
}
...
export default function LineChart({ selectedStock }) {
  ...
  useEffect(() => {
    ...
    if (!yMount || !plotMount || layout.plotViewportW < 1 || !selectedStock) return;
    ...
    async function loadAndDraw() {
      ...
      let data;
      try {
        const payload = await fetchStockPrices(selectedStock);
        if (cancelled) return;
        data = recordsToChartRows(payload.records);
        console.log("Loaded prices:", selectedStock, data.length);
      } catch {
      ...
      const marginPlot = { top: 8, right: 12, bottom: 80, left: 0 };
      const marginY = { top: 8, bottom: 80 };
      ...
      function drawWithXScale(xz, zoomK = 1) {
        const tickCount = getTickCount(zoomK, innerWidth);
        const rotation = getLabelRotation(zoomK);

        xAxisG.call(d3.axisBottom(xz).ticks(tickCount).tickFormat(getDateFormatter(zoomK)));
        xAxisG.selectAll("path,line").attr("stroke", "#94a3b8");

        const tickText = xAxisG
          .selectAll("text")
          .attr("fill", "#64748b")
          .attr("font-size", zoomK < 2 ? 10 : 9);

        if (rotation === 0) {
          tickText.attr("transform", null).style("text-anchor", "middle");
        } else {
          tickText.attr("transform", `rotate(${rotation})`).style("text-anchor", "end");
        }
      ...
      const zoom = d3
        ...
        .on("zoom", (event) => {
          ...
          drawWithXScale(xz, event.transform.k);
        });
  ...
  return (
    <div className="flex w-full min-w-0 flex-col gap-3 rounded-xl border border-slate-200/90 bg-white p-3 shadow-sm">
      ...
      {loadError && <p className="text-sm text-amber-800">Could not load data from backend.</p>}
