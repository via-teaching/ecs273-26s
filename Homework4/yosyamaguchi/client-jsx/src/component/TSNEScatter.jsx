...
export function TSNEScatter({ data: tsneData, selectedStock }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      if (!containerRef.current || !svgRef.current || !tsneData || tsneData.length === 0) {
        return;
      }

      // propsで渡されたデータをd3が扱える形式に変換
      const data = tsneData.map((d) => {
        const ticker = d.Stock;
        return {
          ticker,
          x: d.x,
          y: d.y,
          sector: sectorByTicker[ticker] || "Other",
        };
      });
