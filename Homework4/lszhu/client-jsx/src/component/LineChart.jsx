...
// set API base to fetch stock data from our backend instead of csv files
...
// Fetch stock data for the given ticker and reformat it for the chart instead of fetch from csv
async function loadData(ticker) {
    const response = await fetch(API_BASE + '/stock/' + ticker);
    if (!response.ok) return [];
    const json = await response.json();
    return json.stock_series.map(function(row) {
        return {
...
export function LineChart() {
    ...
    useEffect(function() {
        ...
        const resizeObserver = new ResizeObserver(
            debounce(function(entries) {
                for (const entry of entries) {
                    ...
                    loadData(ticker).then(function(data) {
        ...
        const ticker = getTicker();
        loadData(ticker).then(function(data) {
            if (rect.width && rect.height && !isEmpty(data)) {
...
function drawChart(svgElement, data, width, height, ticker) {
    ...
    stockSelect.on('change', function(event) {
        ...
        loadData(newTicker).then(function(newData) {
