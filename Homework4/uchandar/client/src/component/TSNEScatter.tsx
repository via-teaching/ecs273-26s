...
export default function TSNEScatter({
  ...
  useEffect(() => {
    fetch(`${API}/tsne/`)
      .then(res => res.json())
      .then((rows: { Stock: string; x: number; y: number; sector: string }[]) => {
        const parsed: TsnePoint[] = rows.map(r => ({
          ticker: r.Stock,
          x: r.x,
          y: r.y,
          sector: r.sector,
...
function drawChart(
  ...
  const dots = dotsGroup.selectAll<SVGCircleElement, TsnePoint>("circle")
    .data(data)
    .enter()
    .append("circle")
  ...
  const tickerLabels = labelsGroup.selectAll<SVGTextElement, TsnePoint>("text")
  ...
  sectors.forEach((s, i) => {
    ...
    g.append("rect").attr("width", 12).attr("height", 12).attr("fill", colorScale(s) as string);
