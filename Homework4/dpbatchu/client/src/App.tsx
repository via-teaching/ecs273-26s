...
export default function App() {
  ...
  useEffect(() => {
    fetchStocks()
      ...
      .catch((e: unknown) => {
        setApiError(e instanceof Error ? e.message : "Could not reach backend.");
      });
  }, []);
    ...
    <div className="app-shell">
      ...
      <header className="app-header">
        ...
        <div className="header-center">
          ...
          {apiError ? (
            <span style={{ color:"#ef4444", fontFamily:"'IBM Plex Mono',monospace", fontSize:12 }}>
              ⚠ Backend unreachable
            </span>
          ) : stocks.length === 0 ? (
            <span style={{ color:"#6b7280", fontFamily:"'IBM Plex Mono',monospace", fontSize:12 }}>
              Loading stocks…
            </span>
          ) : (
      ...
      <main className="main-grid">
        ...
        <section className="panel panel-linechart">
          <div className="panel-header">
            ...
            <h2 className="panel-title">
              Price Overview — <span className="stock-name">{selectedStock}</span>
            </h2>
          </div>
          <div className="panel-body">
            {selectedStock && <LineChart stock={selectedStock} />}
          </div>
        </section>
        ...
        <section className="panel panel-news">
          <div className="panel-header">
            ...
            <h2 className="panel-title">
              News Feed — <span className="stock-name">{selectedStock}</span>
            </h2>
          </div>
          <div className="panel-body panel-body-scroll">
            {selectedStock && <NewsList stock={selectedStock} />}
          </div>
        </section>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stock-selector {
          ...
          padding:6px 36px 6px 12px; border-radius:4px;
        ...
        .panel-label {
          ...
          letter-spacing:0.1em; background:rgba(0,212,255,0.1);
          padding:2px 6px; border-radius:2px;
        }
        .panel-title { font-size:13px; font-weight:600; color:var(--text-muted); letter-spacing:0.03em; }
        .stock-name  { color:var(--accent); }
        .panel-body  { flex:1; overflow:hidden; position:relative; }
        .panel-body-scroll { overflow-y:auto; overflow-x:hidden; }
