...
Navigate to the ajoyuan/server folder using the bash terminal
...
### 6: Access the FastAPI

In your web-browser, Use this link:

http://localhost:8000/docs 

to access the FastAPI where you can look at the data

# FRONTEND SETUP

### 5: Install necessary dependencies for client
Open a new bash terminal and navigate to the ajoyuan/client-jsx folder
...
Stock List Endpoint: Fetches the available stock tickers from the database for the frontend dropdown menu

t-SNE Endpoint: Fetches the 2D coordinates for all tracked stocks to plot on an interactive cluster visualization layout on the frontend where a selected stock's point is highlighted.

Stock Prices Endpoint: Retrieves the pricing series for a specific stock ticker to render its line chart.
