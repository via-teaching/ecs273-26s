...
import { StockSeriesUnit, Margin, ComponentSize } from '../types';

const METRICS = ['Open', 'High', 'Low', 'Close'] as const;
const COLORS: Record<string, string> = {
  Open: '#2196F3', High: '#4CAF50', Low: '#F44336', Close: '#FF9800',
};
...
export function LineChart({ selectedStock }: { selectedStock: string }) {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/stock/${selectedStock}`)
      .then((r) => r.json())
      .then((res: { name: string; stock_series: StockSeriesUnit[] }) => {
        const parsed: ParsedRow[] = res.stock_series.map((d) => ({
...
function drawChart(svgEl: SVGSVGElement, data: ParsedRow[], width: number, height: number) {
  ...
  const yScale = d3.scaleLinear()
    .domain([yExtent[0] * 0.98, yExtent[1] * 1.02]).range([innerH, 0]);
  ...
  const lineGen = (metric: keyof Omit<ParsedRow, 'date'>) =>
    d3.line<ParsedRow>().x((d) => xScale(d.date)).y((d) => yScale(d[metric]));

  METRICS.forEach((m) => {
    linesG.append('path').datum(data)
      .attr('fill', 'none').attr('stroke', COLORS[m]).attr('stroke-width', 1.5)
      .attr('class', `line-${m}`).attr('d', lineGen(m));
  });

  const legendG = root.append('g')
    .attr('transform', `translate(${innerW + 10}, ${innerH / 2 - 38})`);
  METRICS.forEach((m, i) => {
    legendG.append('rect').attr('x', 0).attr('y', i * 20)
      .attr('width', 12).attr('height', 12).attr('fill', COLORS[m]);
    legendG.append('text').attr('x', 17).attr('y', i * 20 + 10)
    ...
    .on('zoom', (event) => {
      ...
      METRICS.forEach((m) => {
        const newLine = d3.line<ParsedRow>()
          .x((d) => xNew(d.date)).y((d) => yScale(d[m]));
        linesG.select(`.line-${m}`).attr('d', newLine);
      });
    });

  d3.select(svgEl).call(zoom.filter((e) => !e.ctrlKey && !e.button));

  d3.select(svgEl).on('wheel.zoom', function (event: WheelEvent) {
    event.preventDefault();
    const t = d3.zoomTransform(svgEl);
    if (event.shiftKey) {
      d3.select(svgEl).call(zoom.transform, t.translate(-event.deltaY, 0));
    } else {
      const delta = -event.deltaY * 0.002;
      const newK = Math.max(1, Math.min(20, t.k * Math.exp(delta)));
      const mx = event.offsetX - margin.left;
      d3.select(svgEl).call(zoom.transform,
        d3.zoomIdentity.translate(t.x + (mx - t.x) * (1 - newK / t.k), 0).scale(newK));
    }
