...
function drawChart(svgElement, data, width, height) {
  ...
  const xAxisGroup = svg
    ...
    .call(
      d3
        ...
        .ticks(6)
        .tickFormat((d) => {
          const daysDiff = (xScale.domain()[1] - xScale.domain()[0]) / (1000 * 60 * 60 * 24);
          if (daysDiff <= 14) return d3.timeFormat("%b %d")(d);
          if (daysDiff <= 90) return d3.timeFormat("%b %d '%y")(d);
  ...
  const zoom = d3
    ...
    .on("zoom", (event) => {
      ...
      xAxisGroup.call(
        d3.axisBottom(newXScale).ticks(6).tickFormat((d) => {
          const daysDiff = (newXScale.domain()[1] - newXScale.domain()[0]) / (1000 * 60 * 60 * 24);
          if (daysDiff <= 14) return d3.timeFormat("%b %d")(d);
          if (daysDiff <= 90) return d3.timeFormat("%b %d '%y")(d);
          return d3.timeFormat("%b '%y")(d);
        })
