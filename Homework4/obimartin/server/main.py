...
db = client.stock_obimartin # set my name
...
async def get_stock_news(stock_name: str) -> list[StockNewsModel]:
    ...
    news = await news_collection.find({"Stock": stock_name}).sort("Date", 1).to_list(length=None)
    return news # returns news from the database
...
async def get_tsne() -> list[tsneDataModel]:
    ...
    return tsne_data #return t-SNE points as a list
