...
export function TSNEScatterPlot({ selectedStock, apiBase }) {
  const containerRef = useRef(null);
  const svgRef = useRef(null);

  useEffect(() => {
    if (!containerRef.current || !svgRef.current || !selectedStock || !apiBase) {
      return;
    }

    async function loadAndDraw(width, height) {
      try {
        const response = await fetch(`${apiBase}/tsne`);

        if (!response.ok) {
          throw new Error("Failed to fetch t-SNE data from backend");
        }
        ...
        console.error("Failed to load t-SNE data from backend", error);
      }
    }
...
function drawScatter(svgElement, data, width, height, selectedStock) {
  ...
  function updatePoints(currentXScale, currentYScale) {
    pointsGroup
      ...
      .attr("r", (d) => (d.Stock === selectedStock ? 9 : 5))
      .attr("fill", (d) => colorScale(d.Sector))
      .attr("stroke", (d) => (d.Stock === selectedStock ? "black" : "white"))
      .attr("stroke-width", (d) => (d.Stock === selectedStock ? 2.5 : 1))
      .attr("opacity", 0.85);

    pointsGroup
      .selectAll("text.stock-label")
      .data(
        data.filter((d) => d.Stock === selectedStock),
        (d) => d.Stock
      )
      .join("text")
      .attr("class", "stock-label")
      .attr("x", (d) => currentXScale(d.x) + 10)
      .attr("y", (d) => currentYScale(d.y) - 10)
      .text((d) => d.Stock)
      .style("font-size", "0.8rem")
      .style("font-weight", "bold");
  }
