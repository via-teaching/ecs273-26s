...
export default function TSNEScatter({ selectedStock }: TSNEScatterProps) {
  ...
  useEffect(() => {
    ...
    async function loadPoints() {
      ...
      try {
        const response = await fetch(`${API_BASE}/tsne`);
        if (!response.ok) {
          throw new Error("Failed to load t-SNE data.");
        }

        const data: TSNEPoint[] = await response.json();
        if (cancelled) return;
        setPoints(data);
      } catch (err) {
        if (cancelled) return;
        setPoints([]);
        setError(err instanceof Error ? err.message : "Failed to load t-SNE data.");
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
  ...
  if (loading) {
    return <div className="flex h-full items-center justify-center text-gray-500">Loading t-SNE data...</div>;
  }
...
function drawScatter(
  ...
  const renderPoints = (
    ...
    labels
      .selectAll<SVGTextElement, (typeof points)[number]>("text")
      .data(
