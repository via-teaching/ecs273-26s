...
export default function ScatterPlot({ ticker, onSelect }) {
  ...
  // Fetch t-SNE data from the backend on mount
  useEffect(() => {
    fetch("http://127.0.0.1:8000/tsne")
      ...
      .then((json) => {
        // API returns: [{Stock, x, y, sector}, ...]
        // Rename `Stock` -> `ticker` so existing chart code works unchanged.
        const adapted = json.map((d) => ({
          ticker: d.Stock,
          x: +d.x,
          y: +d.y,
          sector: d.sector,
  ...
  useEffect(() => {
    ...
    const SECTORS = Array.from(new Set(tsneData.map((d) => d.sector))).sort();
