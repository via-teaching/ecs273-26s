...
// load all csvs at build time
const csvFiles = import.meta.glob('/data/stockdata/*.csv', { query: '?raw', import: 'default' })
...
export default function LineChart({ ticker }: { ticker: string }) {
  ...
  useEffect(() => {
    const path = `/data/stockdata/${ticker}.csv`
    if (!csvFiles[path]) return
    csvFiles[path]().then((raw) => {
      const parsed = d3.csvParse(raw as string, (d) => ({
