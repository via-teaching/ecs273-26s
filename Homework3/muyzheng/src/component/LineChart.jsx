...
export function LineChart({ stock }) {
  ...
  useEffect(() => {
    if (!stock) return;
    setData([]);
    d3.csv(`/stockdata/${stock}.csv`).then((raw) => {
  ...
  useEffect(() => {
    ...
    xAxisG.call(d3.axisBottom(xScale).ticks(Math.max(4, Math.floor(innerW / 80))).tickFormat(d3.timeFormat("%b %d")))
    ...
    const legendY = 4;
    const legendBg = root.append("g").attr("transform", `translate(${legendX},${legendY})`);
    ...
    LINES.forEach(({ label, color }, i) => {
      const g = legendBg.append("g").attr("transform", `translate(-86,${i * 18 + 4})`);
    ...
    const zoom = d3.zoom()
      ...
      .on("zoom", (event) => {
        ...
        xAxisG.call(
          d3.axisBottom(newX).ticks(Math.max(4, Math.floor(innerW / 80))).tickFormat(d3.timeFormat("%b %d"))
        ).selectAll("text").attr("transform", "rotate(-35)").style("text-anchor", "end");
        LINES.forEach(({ key }) => {
          chartArea.select(`.line-${key}`)
            .attr("d", d3.line().x((d) => newX(d.date)).y((d) => yScale(d[key])).curve(d3.curveMonotoneX)(data));
        });
      });
  ...
  return (
    <div ref={wrapperRef} style={{ width: "100%", height: "100%", overflowX: "auto" }}>
