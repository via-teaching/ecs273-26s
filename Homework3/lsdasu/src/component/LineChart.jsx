...
const allCsvFiles = import.meta.glob('../../data/stockdata/*.csv', { query: '?raw', eager: true });
...
export function LineChart({ selectedStock }) {
  ...
  useEffect(() => {
    ...
    const key = Object.keys(allCsvFiles).find(k => k.endsWith(`/${selectedStock}.csv`));
    if (!key) return;

    const raw = allCsvFiles[key].default ?? allCsvFiles[key];
