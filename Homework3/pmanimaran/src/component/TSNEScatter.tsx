...
import { TSNEPoint, Margin, ComponentSize } from '../types';
...
export function TSNEScatter({ selectedStock }: { selectedStock: string }) {
  ...
  const dataRef = useRef<TSNEPoint[]>([]);
  const [size, setSize] = useState<ComponentSize>({ width: 0, height: 0 });
  const [dataLoaded, setDataLoaded] = useState(false);

  useEffect(() => {
    d3.csv('/data/tsne.csv').then((rows) => {
      dataRef.current = rows as unknown as TSNEPoint[];
      setDataLoaded(true);
    });
  }, []);
...
function drawScatter(
  ...
  const sorted = [...data].sort((a) => (a.ticker === selected ? 1 : -1));
  ...
  const sel = data.find((d) => d.ticker === selected);
  if (sel) {
    pointsG.append('text')
      .attr('x', xScale(+sel.x))
      .attr('y', yScale(+sel.y) - 14)
  ...
  sectors.forEach((s, i) => {
    legendG.append('circle')
      .attr('cx', 6).attr('cy', i * 20 + 10)
      .attr('r', 5).attr('fill', SECTOR_COLORS(s));
    legendG.append('text')
      .attr('x', 16).attr('y', i * 20 + 14)
