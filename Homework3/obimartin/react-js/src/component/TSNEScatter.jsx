...
const tsneModule = import.meta.glob("../../data/tsne.csv", {
  query: "?raw",
  import: "default",
});
...
const SECTOR_COLORS = {
  ...
  "Energy":                 "#ecb623",
  "Healthcare":             "#f4895f",
  "Consumer Staples":       "#de324c",
  "Consumer Discretionary": "#6c584c",
};
...
export function TSNEScatter({ stock, onStockSelect }) {
  ...
  useEffect(() => {
    const key = "../../data/tsne.csv";
    if (!tsneModule[key]) return;
    tsneModule[key]().then((text) => {
...
function drawChart(svgElement, data, selectedStock, onStockSelect, width, height) {
  ...
  chartArea
    ...
    .attr("fill", d => SECTOR_COLORS[d.sector] ?? "#aaa")
    .attr("stroke", "none")
